...
export type AiWorkerRequest = { id: number; gs: GameState };
export type AiWorkerResponse = { id: number; next: GameState };
...
ctx.onmessage = (ev: MessageEvent<AiWorkerRequest>) => {
  ...
  ctx.postMessage({ id, next } satisfies AiWorkerResponse);
};
